...
{
    [Info("RemoteDoors", "Reneb", "1.0.5", ResourceId = 1379)]
    ...
    { 
        ...
        static string permissionRemoteDoors = "canremoteactivate";
        ...
        void Init()
        {
            CheckCfg<string>("Permission - Oxide Permissions", ref permissionRemoteDoors);
            CheckCfg<Dictionary<string,object>>("Remote Activator - Cost", ref cost);
            CheckCfg<int>("Remote Activator - Max Door Distance", ref maxDistance);
            CheckCfg<int>("Remote Activator - Max Doors", ref maxDoors);
            CheckCfg<int>("Remote Activator - Anti Trap Distance", ref antiTrapDistance);
            SaveConfig();
        }
        ...
        static int signColl = UnityEngine.LayerMask.GetMask(new string[] { "Deployed" });
        ...
        public class RemoteDoor
        {
            ...
            public bool OpenDoor(bool openclose)
            {
                ...
                if (openclose)
                {
                    updatelayer.Invoke(door, new object[] { });
        ...
        {
            var newItem = ItemManager.CreateByName(prefab, 1);
            ...
            sign.textureID = FileStorage.server.Store(Convert.FromBase64String(SignTexture()), FileStorage.Type.png, sign.net.ID);

            newBaseEntity.SetFlag(BaseEntity.Flags.Locked, true);
            newBaseEntity.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
        }
        ...
        void ShowUI(BasePlayer player, Vector3 remotePos, string ttype)
        {
            ...
            var doverlay = doorsoverlay.Replace("{remoteid}", string.Format("'{0}' '{1}' '{2}'", remotePos.x.ToString(), remotePos.y.ToString(), remotePos.z.ToString()));
            CommunityEntity.ServerInstance.ClientRPCEx(new Network.SendInfo() { connection = player.net.connection }, null, "AddUI", new Facepunch.ObjectList(doverlay, null,null,null,null));
            if(ttype == "admin")
            {
                var aoverlay = adminoverlay.Replace("{remoteid}", string.Format("'{0}' '{1}' '{2}'", remotePos.x.ToString(), remotePos.y.ToString(), remotePos.z.ToString()));
                CommunityEntity.ServerInstance.ClientRPCEx(new Network.SendInfo() { connection = player.net.connection }, null, "AddUI", new Facepunch.ObjectList(aoverlay, null, null, null, null));
            }
            if(allowAuth.ContainsKey(remotePos) && (UnityEngine.Time.realtimeSinceStartup - allowAuth[remotePos] <= 15))
            {
                var goverlay = getaccessoverlay.Replace("{remoteid}", string.Format("'{0}' '{1}' '{2}'", remotePos.x.ToString(), remotePos.y.ToString(), remotePos.z.ToString()));
                CommunityEntity.ServerInstance.ClientRPCEx(new Network.SendInfo() { connection = player.net.connection }, null, "AddUI", new Facepunch.ObjectList(goverlay, null, null, null, null));
            }
        }
        ...
        void OnUseSignage(BasePlayer player, Signage sign)
        {
            ...
            else if (remoteActivators[targetPos].autorizedUsers.Contains(player.userID.ToString()) || (allowAuth.ContainsKey(targetPos) && (UnityEngine.Time.realtimeSinceStartup - allowAuth[targetPos] <= 15)))
        ...
        {
            RaycastHit hit;
            if (!UnityEngine.Physics.Raycast(ray, out hit, 3f, currentCol))
                return null;
        ...
        void ccmdRemoteCommand(ConsoleSystem.Arg arg)
        {
            ...
            switch (arg1)
            {
                ...
                case "access":
                    ...
                    if(UnityEngine.Time.realtimeSinceStartup - allowAuth[targetPos] > 15)
                    {
                        SendReply(player, "You are not allowed to get access to this remote activator.");
                        return;
                    }
                    ...
                    if(Vector3.Distance(remoteActivators[targetPos].Pos(), player.transform.position) > 3f)
                    {
                        SendReply(player, "You are too far from the remote activator." + Vector3.Distance(remoteActivators[targetPos].Pos(), player.transform.position).ToString());
                        return;
                    }
                    storedData.RemoteActivators.Remove(remoteActivators[targetPos]);
                ...
                case "giveaccess":
                    ...
                    allowAuth.Add(targetPos, UnityEngine.Time.realtimeSinceStartup);
        ...
        string GetCostInString()
        {
            ...
            foreach(KeyValuePair<string,object> pair in cost)
            {
                coststring += string.Format("{0}x {1} - ", pair.Value.ToString(), pair.Key); 
            }
            return coststring;
        }
        void SendHelp(BasePlayer player, bool full = true)
        {
            ...
            if (full)
            {
                ...
                text += string.Format("\nYou may have {0} doors per remote, max {1}m away from the remote.", maxDoors.ToString(), maxDistance.ToString());
            }
